...
alter table complex add foreign key (address_id) references ComplexesRegions(id)
alter table complex add address_id int;
select * from complex

go
declare @JSON nvarchar(max)
select @JSON = BulkColumn from openrowset (bulk 'C:/Users/Andrew/Desktop/6_sem/ад/4/complexes.json', single_clob) as json
...
create or alter procedure GetUniont (@first nvarchar(255), @second nvarchar(255), @union geography output)
as
begin
	DECLARE @firstd geography;  
	DECLARE @secondd geography;  
